...
>
  Logout
      ...
      <div
        ...
        bg-gradient-to-b from-gray-900 to-black text-white z-50
        transform transition-transform duration-300
        ${isSidebarOpen ? "translate-x-0" : "-translate-x-full"}`}
      >
        ...
        <div className="p-4 overflow-y-auto">
          <Link
            to="/"
            onClick={() => setIsSidebarOpen(false)}
            className="block px-4 py-3 rounded-lg hover:bg-pink-900/40"
          >
            <FaHome /> Home
          </Link>

          <div className="mt-6">
            <h3 className="text-pink-400 px-4 mb-2">Categories</h3>
            {categories.map((c) => (
              <button
                key={c.type}
                onClick={() => handleCategoryClick(c.type)}
                className="w-full px-4 py-2 flex gap-3 hover:bg-pink-900/30"
              >
                {c.icon} {c.name}
              </button>
            ))}
          ...
          <div className="mt-6">
            <h3 className="text-pink-400 px-4 mb-2">Genres</h3>
            {genresLoading ? (
              <Loader />
            ) : (
              genres.map((g) => (
